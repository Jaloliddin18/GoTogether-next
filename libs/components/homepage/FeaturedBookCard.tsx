...
const FeaturedBookCard = (props: FeaturedBookCardProps) => {
	...
	const card = (
		<MuiBox className="top-card-box" onClick={() => pushDetailHandler(book._id)} sx={cardSx}>
			<MuiBox sx={imageSx}>
				<MuiBox sx={categoryBadgeSx}>{book.bookCategory?.replace(/_/g, ' ')}</MuiBox>
				{book.isBorrowable && <MuiBox sx={borrowableBadgeSx}>Borrowable</MuiBox>}
			</MuiBox>
			<MuiBox sx={{ padding: '14px 16px', display: 'flex', flexDirection: 'column' }}>
				<MuiTypography sx={titleSx}>{book.bookTitle}</MuiTypography>
				<MuiTypography sx={{ ...metaTextSx, mb: '10px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
					{book.bookAuthor}
				</MuiTypography>
				<MuiBox sx={{ display: 'flex', alignItems: 'center', gap: '12px', mb: '10px' }}>
					...
					<MuiBox sx={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
						...
						<MuiTypography sx={{ fontSize: '12px', color: '#111827', fontWeight: 700 }}>
